...
**Standalone (non-stacked) fix PRs on wolfy** — branched off `wolfy/main` directly, NOT part of the Tier stack: **PR #17 `fix/scan-disc-not-ready-retry`** (rc2 field bug — scan DISC_OK-but-not-data-readable race; open, non-draft, base `main`). See [[project_rc2_field_bug_report]]. Merge order is forced bottom-up by the Alembic migration chain (…→0019→0020) + the shared openapi snapshot; retarget each PR to `main` as the one below it merges (per CLAUDE.local.md).
