...
description: Current state of the 8-deep neu-port PR stack on wolfy (#6–#13), what's shipped, and the backlog pull queue
metadata:
  type: project
---
...
All 8 are **DRAFT, unmerged**. PR titles are functionality-first `Tier-N — <what it does>` (no "neu ports"/"quick-wins"). Merge order is forced bottom-up by the Alembic migration chain (0013→0018) + the shared openapi snapshot; retarget each PR to `main` as the one below it merges (per CLAUDE.local.md).

**Shipped backlog items (see `../arm-ai/arm-v3/docs/port-backlog.md`, the pull queue):** B3, B5, B30, and the correctness sweep B25/B26/B29. B0 retired. Everything else (B1/B2/B4/B6–B24/B27/B28) still `ready`/`blocked`.
