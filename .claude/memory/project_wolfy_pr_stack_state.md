...
All of #6–#16 are **DRAFT, unmerged**; Tier-12 (`feat/track-operator-editing`) is pushed to both remotes but its PR is not yet opened (open an in-repo DRAFT PR on wolfy targeting `feat/drives-enrichment` when ready). PR titles are functionality-first `Tier-N — <what it does>` (no "neu ports"/"quick-wins").

**Standalone (non-stacked) fix PRs on wolfy** — branched off `wolfy/main` directly, NOT part of the Tier stack: **PR #17 `fix/scan-disc-not-ready-retry`** (rc2 field bug — scan DISC_OK-but-not-data-readable race) — **MERGED to wolfy/main 2026-06-14** (`30bec7e6`). See [[project_rc2_field_bug_report]]. Merge order for the *stack* is forced bottom-up by the Alembic migration chain (…→0019→0020) + the shared openapi snapshot; retarget each PR to `main` as the one below it merges (per CLAUDE.local.md). **Note:** PR #17's merge moved `wolfy/main` ahead of the stack base (the stack #6 was built on the old main `efa51c6c`; new main `30bec7e6` = old main + the rc2 fix) — the stack is behind main by exactly the rc2 fix, harmless until the stack rebases/merges.

**UI-PORT BACKEND TARGET — `integration/ui-port-target` (origin, `570311a3`).** A non-PR integration branch = new `wolfy/main` (incl. the merged rc2 fix) + a merge of the stack tip (`feat/track-operator-editing`, all 11 tiers #6–#16). Clean auto-merge, **full suite 1287 green**. This is the assembled all-tiers backend the UI port runs against (the port can't run on bare wolfy/main, which lacks the unmerged tiers — notably notifications/Tier-3). Rebuild after a new tier lands: re-merge the new stack tip into it. A UI-port BLOCKER AUDIT (2026-06-14) found **NO backend feature blockers** — the only apparent one (notifications) is built+stacked (Tier-3/#8); auth/login is ready; themes (B9 unbuilt) + image-proxy (B13 deferred) degrade gracefully in the UI; transcoder/config/drives/tracks/naming shipped with field-rename adapters (BC-S5–S13); dashboard composes client-side (B14 wontfix). So the port can start now against this branch.
